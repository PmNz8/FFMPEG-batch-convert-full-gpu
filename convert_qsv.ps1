...
while ($path -eq "" -or -not (Test-Path $path)) {
    ...
    else
    {
        ...
        $file_list = Get-ChildItem -Path $path -Include *.mkv, *.mp4 -File -Recurse -Exclude *hevc*
        ...
        foreach($file in $file_list)
        {
            ...
            & $ffmpeg\ffmpeg.exe -y -init_hw_device d3d11va=dx11:,vendor=0x8086 -init_hw_device qsv=qs@dx11 -filter_hw_device qs -hwaccel d3d11va -hwaccel_output_format d3d11 -i file:$($file.FullName) -map 0 -c copy -map -0:v:1 -threads 0 -codec:v:0 hevc_qsv -low_power 0 -preset medium -global_quality 21 -look_ahead 1 -look_ahead_depth 80 -vf "hwmap=derive_device=qsv,scale_qsv=format=p010" -profile:v main10 "$($targetFull)\$($addon)"
